...
### Step 3: Fitting Natural and Quartic Splines to the Tanager Data

```{r, message=FALSE, warning=FALSE}
### Step 3: Fitting Natural and Quartic Splines to the Tanager Data

# Function to fit a natural spline and a quartic spline, returning the models, coefficients, goodness-of-fit metrics, and RGB colors
fit_splines <- function(wl, reflectance) {
  # Fit natural quartic spline
  natural_spline_fit <- lm(reflectance ~ ns(wl, df = 5))
  
  # Fit quartic spline
  quartic_spline_fit <- lm(reflectance ~ poly(wl, 4, raw = TRUE))
  
  # Convert fitted splines to rspec and then to RGB hex codes
  natural_spline_rspec <- as.rspec(data.frame(wl = wl, reflectance = predict(natural_spline_fit)), lim = c(300, 700))
  natural_rgb <- spec2rgb(natural_spline_rspec)
  
  quartic_spline_rspec <- as.rspec(data.frame(wl = wl, reflectance = predict(quartic_spline_fit)), lim = c(300, 700))
  quartic_rgb <- spec2rgb(quartic_spline_rspec)
  
  # Extract coefficients
  natural_coef <- round(coef(natural_spline_fit), 2)
  quartic_coef <- round(coef(quartic_spline_fit), 2)
  
  # Calculate goodness-of-fit metrics
  natural_adj_r2 <- summary(natural_spline_fit)$adj.r.squared
  quartic_adj_r2 <- summary(quartic_spline_fit)$adj.r.squared
  
  natural_aic <- AIC(natural_spline_fit)
  quartic_aic <- AIC(quartic_spline_fit)
  
  return(list(
    natural_spline_fit = natural_spline_fit,
    quartic_spline_fit = quartic_spline_fit,
    natural_rgb = natural_rgb,
...
Step 3b--making the species plots of natural and quartic spline
...
plot(wl, reflectance, type = "p", pch = 16, cex = 0.5,  # Adjusted size of points
     col = adjustcolor("black", alpha.f = 0.5),  # Adjusted transparency of points
     main = paste("Quartic Spline -", species_name),
     xlab = "Wavelength (nm)", ylab = "Reflectance")
lines(wl, predict(result$quartic_spline_fit), col = rgb_colors[[species_name]], lwd = 2)
mtext(paste("Adj R2:", round(result$quartic_adj_r2, 3), "AIC:", round(result$quartic_aic, 2)),
      side = 3, line = 0.5, cex = 0.8, col = "blue")
}
...
pdf("natural_vs_quartic_splines_with_metrics.pdf", width = 12, height = 8)
for (species_name in names(spline_results)) {
  plot_splines_with_metrics(species_name, tanData_filtered$wl, tanData_filtered[[species_name]], spline_results, rgb_colors)
}
dev.off()
...
# Calculate the maximum reflectance across all splines
max_reflectance <- max(sapply(spline_results, function(result) {
  max(predict(result$natural_spline_fit), predict(result$quartic_spline_fit))
}))

# Initialize vectors to store average splines and standard deviations
natural_spline_avg <- numeric(length(tanData_filtered$wl))
quartic_spline_avg <- numeric(length(tanData_filtered$wl))
natural_spline_sd <- numeric(length(tanData_filtered$wl))
quartic_spline_sd <- numeric(length(tanData_filtered$wl))

# Calculate averages
for (result in spline_results) {
  natural_pred <- predict(result$natural_spline_fit)
  quartic_pred <- predict(result$quartic_spline_fit)
  
  natural_spline_avg <- natural_spline_avg + natural_pred
  quartic_spline_avg <- quartic_spline_avg + quartic_pred
}

# Convert sums to averages
natural_spline_avg <- natural_spline_avg / length(spline_results)
quartic_spline_avg <- quartic_spline_avg / length(spline_results)
...
for (result in spline_results) {
  ...
  natural_spline_sd <- natural_spline_sd + (natural_pred - natural_spline_avg)^2
  quartic_spline_sd <- quartic_spline_sd + (quartic_pred - quartic_spline_avg)^2
}

natural_spline_sd <- sqrt(natural_spline_sd / (length(spline_results) - 1))
quartic_spline_sd <- sqrt(quartic_spline_sd / (length(spline_results) - 1))

### Correct for negative values in the average splines

# Convert the average natural and quartic splines to rspec objects
natural_spline_rspec <- as.rspec(data.frame(wl = tanData_filtered$wl, reflectance = natural_spline_avg), lim = c(300, 700))
quartic_spline_rspec <- as.rspec(data.frame(wl = tanData_filtered$wl, reflectance = quartic_spline_avg), lim = c(300, 700))

# Correct any negative values in the rspec objects
natural_spline_rspec <- procspec(natural_spline_rspec, fixneg = "zero")
quartic_spline_rspec <- procspec(quartic_spline_rspec, fixneg = "zero")

# Convert corrected rspec objects to RGB hex codes
natural_spline_rgb <- spec2rgb(natural_spline_rspec)
quartic_spline_rgb <- spec2rgb(quartic_spline_rspec)
...
# Create a plot for the average quartic spline with confidence intervals
plot(tanData_filtered$wl, quartic_spline_avg, type = "l", col = quartic_spline_rgb, lwd = 4,
     main = "Average Quartic Spline with Variation Hotspots",
     xlab = "Wavelength (nm)", ylab = "Reflectance", ylim = c(0, max_reflectance))
polygon(c(tanData_filtered$wl, rev(tanData_filtered$wl)), 
        c(quartic_spline_avg + quartic_spline_sd, rev(quartic_spline_avg - quartic_spline_sd)), 
        col = adjustcolor(quartic_spline_rgb, alpha.f = 0.2), border = NA)
lines(tanData_filtered$wl, quartic_spline_avg, col = quartic_spline_rgb, lwd = 4)
...
# Average Quartic Spline with Hotspots in PDF
plot(tanData_filtered$wl, quartic_spline_avg, type = "l", col = quartic_spline_rgb, lwd = 4,
     main = "Average Quartic Spline with Variation Hotspots",
     xlab = "Wavelength (nm)", ylab = "Reflectance", ylim = c(0, max_reflectance))
polygon(c(tanData_filtered$wl, rev(tanData_filtered$wl)), 
...
### Step 4b: Plot Natural Spline Mean and Quartic Spline Mean with Species Overlaid
...
# Plot Quartic Spline Mean with Species Overlaid
plot(tanData_filtered$wl, quartic_spline_avg, type = "l", col = quartic_spline_rgb, lwd = 4,
     main = "Quartic Spline Mean with Species Overlaid",
     xlab = "Wavelength (nm)", ylab = "Reflectance", ylim = c(0, max_reflectance))
for (i in seq_along(spline_results)) {
  species_rspec <- as.rspec(data.frame(wl = tanData_filtered$wl, reflectance = predict(spline_results[[i]]$quartic_spline_fit)), lim = c(300, 700))
  species_rspec <- procspec(species_rspec, fixneg = "zero")  # Correct any negative values
  species_rgb <- spec2rgb(species_rspec)
  lines(tanData_filtered$wl, predict(spline_results[[i]]$quartic_spline_fit), col = adjustcolor(species_rgb, alpha.f = 0.3))
}
lines(tanData_filtered$wl, quartic_spline_avg, col = quartic_spline_rgb, lwd = 4)
...
# Plot Quartic Spline Mean with Species Overlaid in PDF
plot(tanData_filtered$wl, quartic_spline_avg, type = "l", col = quartic_spline_rgb, lwd = 4,
     main = "Quartic Spline Mean with Species Overlaid",
     xlab = "Wavelength (nm)", ylab = "Reflectance", ylim = c(0, max_reflectance))
for (i in seq_along(spline_results)) {
...
### Step 4b Make a pair of plots of the natural and quartic spline where the spline is colored as a spec that is converted to its hsv value (make sure no negative values)
...
# Convert the quartic spline to an rspec object and then to an RGB hex code
quartic_spline_rspec <- as.rspec(data.frame(wl = tanData_filtered$wl, reflectance = quartic_spline_avg), lim = c(300, 700))
quartic_spline_rgb <- spec2rgb(quartic_spline_rspec)

# Plot the natural and quartic splines side by side in R Markdown
par(mfrow = c(1, 2))  # Arrange plots side by side
...
# Plot Quartic Spline with Species Overlaid
plot(tanData_filtered$wl, quartic_spline_avg, type = "l", col = quartic_spline_rgb, lwd = 4,  # Increased thickness
     main = "Quartic Spline with Species Overlaid",
     xlab = "Wavelength (nm)", ylab = "Reflectance", ylim = c(0, max_reflectance))

# Loop through each species to plot their splines with slightly more visible transparency
for (species in names(spline_results)) {
  species_spline_rspec <- as.rspec(data.frame(wl = tanData_filtered$wl, reflectance = predict(spline_results[[species]]$quartic_spline_fit)), lim = c(300, 700))
  species_rgb <- spec2rgb(species_spline_rspec)
  
  # Adjust transparency to make species lines slightly more visible
  species_rgb_alpha <- adjustcolor(species_rgb, alpha.f = 0.4)  # Adjusted alpha for more visibility
  
  lines(tanData_filtered$wl, predict(spline_results[[species]]$quartic_spline_fit), col = species_rgb_alpha)
}

# Re-plot the quartic spline on top to ensure it stands out
lines(tanData_filtered$wl, quartic_spline_avg, col = quartic_spline_rgb, lwd = 4)  # Increased thickness

# Save the plots to a PDF as well
pdf("natural_and_quartic_splines_with_species_overlay.pdf", width = 12, height = 8)
par(mfrow = c(1, 2))  # Arrange plots side by side in the PDF
...
# Plot Quartic Spline with Species Overlaid in PDF
plot(tanData_filtered$wl, quartic_spline_avg, type = "l", col = quartic_spline_rgb, lwd = 4,
     main = "Quartic Spline with Species Overlaid",
     xlab = "Wavelength (nm)", ylab = "Reflectance", ylim = c(0, max_reflectance))
for (species in names(spline_results)) {
  species_rgb_alpha <- adjustcolor(spec2rgb(as.rspec(data.frame(wl = tanData_filtered$wl, reflectance = predict(spline_results[[species]]$quartic_spline_fit)), lim = c(300, 700))), alpha.f = 0.4)
  lines(tanData_filtered$wl, predict(spline_results[[species]]$quartic_spline_fit), col = species_rgb_alpha)
}
lines(tanData_filtered$wl, quartic_spline_avg, col = quartic_spline_rgb, lwd = 4)
...
for (species in names(spline_results)) {
  ...
  # Plot original data and quartic spline fit
  plot(tanData_filtered$wl, tanData_filtered[[species]], main = paste("Quartic Spline Fit for", species),
       xlab = "Wavelength (nm)", ylab = "Reflectance", type = "o", col = color)
  lines(tanData_filtered$wl, predict(result$quartic_spline_fit), col = "green", lwd = 2)
  mtext(paste("Adj R2:", round(result$quartic_adj_r2, 3), "AIC:", round(result$quartic_aic, 2)),
        side = 3, line = 0.5, cex = 0.7, col = "blue")
}
...
spline_table <- data.frame(
  Species = names(spline_results),
  Natural_Spline_Coefficients = sapply(spline_results, function(x) paste(x$natural_coef, collapse = ", ")),
  Quartic_Spline_Coefficients = sapply(spline_results, function(x) paste(x$quartic_coef, collapse = ", ")),
  Natural_Adj_R2 = sapply(spline_results, function(x) round(x$natural_adj_r2, 3)),
  Quartic_Adj_R2 = sapply(spline_results, function(x) round(x$quartic_adj_r2, 3)),
  Natural_AIC = sapply(spline_results, function(x) round(x$natural_aic, 2)),
  Quartic_AIC = sapply(spline_results, function(x) round(x$quartic_aic, 2))
)
...
for (i in 1:length(spline_results)) {
  ...
  spline_traits[i, "Natural_Spline_3"] = spline_results[[i]]$natural_coef[4]
  spline_traits[i, "Natural_Spline_4"] = spline_results[[i]]$natural_coef[5]
  spline_traits[i, "Quartic_Spline_1"] = spline_results[[i]]$quartic_coef[2]
  spline_traits[i, "Quartic_Spline_2"] = spline_results[[i]]$quartic_coef[3]
  spline_traits[i, "Quartic_Spline_3"] = spline_results[[i]]$quartic_coef[4]
}
...
for (i in 1:1){
  ...
  for (j in 2:41){
    ...
    par(mfrow = c(2, 2))  # Set up for side-by-side plotting: two columns, two rows per page
    
    for (species in names(spline_results_df)) {
      result <- spline_results_df[[species]]
...
df_sequence <- rep(c(1:41),51)

spline_table_df_ordered <- cbind(df_sequence, spline_table_df_ordered)
...
Step 3b--making the species plots of natural and B Spline
...
plot(wl, reflectance, type = "p", pch = 16, cex = 0.5,  # Adjusted size of points
     col = adjustcolor("black", alpha.f = 0.5),  # Adjusted transparency of points
     main = paste("B Spline -", species_name),
     xlab = "Wavelength (nm)", ylab = "Reflectance")
lines(wl, predict(result$b_spline_fit), col = rgb_colors[[species_name]], lwd = 2)
mtext(paste("Adj R2:", round(result$quartic_adj_r2, 3), "AIC:", round(result$quartic_aic, 2)),
      side = 3, line = 0.5, cex = 0.8, col = "blue")
}
...
pdf("natural_vs_b_splines_with_metrics.pdf", width = 12, height = 8)
for (species_name in names(spline_results)) {
  plot_splines_with_metrics(species_name, tanData_filtered$wl, tanData_filtered[[species_name]], spline_results, rgb_colors)
}
dev.off()
...
# Calculate the maximum reflectance across all splines
max_reflectance <- max(sapply(spline_results, function(result) {
  max(predict(result$natural_spline_fit), predict(result$b_spline_fit))
}))

# Initialize vectors to store average splines and standard deviations
natural_spline_avg <- numeric(length(tanData_filtered$wl))
b_spline_avg <- numeric(length(tanData_filtered$wl))
natural_spline_sd <- numeric(length(tanData_filtered$wl))
b_spline_sd <- numeric(length(tanData_filtered$wl))

# Calculate averages
for (result in spline_results) {
  natural_pred <- predict(result$natural_spline_fit)
  quartic_pred <- predict(result$b_spline_fit)
  
  natural_spline_avg <- natural_spline_avg + natural_pred
  b_spline_avg <- b_spline_avg + quartic_pred
}

# Convert sums to averages
natural_spline_avg <- natural_spline_avg / length(spline_results)
b_spline_avg <- b_spline_avg / length(spline_results)
...
for (result in spline_results) {
  ...
  natural_spline_sd <- natural_spline_sd + (natural_pred - natural_spline_avg)^2
  b_spline_sd <- b_spline_sd + (quartic_pred - b_spline_avg)^2
}

natural_spline_sd <- sqrt(natural_spline_sd / (length(spline_results) - 1))
b_spline_sd <- sqrt(b_spline_sd / (length(spline_results) - 1))

### Correct for negative values in the average splines

# Convert the average natural and B Splines to rspec objects
natural_spline_rspec <- as.rspec(data.frame(wl = tanData_filtered$wl, reflectance = natural_spline_avg), lim = c(300, 700))
b_spline_rspec <- as.rspec(data.frame(wl = tanData_filtered$wl, reflectance = b_spline_avg), lim = c(300, 700))

# Correct any negative values in the rspec objects
natural_spline_rspec <- procspec(natural_spline_rspec, fixneg = "zero")
b_spline_rspec <- procspec(b_spline_rspec, fixneg = "zero")

# Convert corrected rspec objects to RGB hex codes
natural_spline_rgb <- spec2rgb(natural_spline_rspec)
b_spline_rgb <- spec2rgb(b_spline_rspec)
...
# Create a plot for the average B Spline with confidence intervals
plot(tanData_filtered$wl, b_spline_avg, type = "l", col = b_spline_rgb, lwd = 4,
     main = "Average B Spline with Variation Hotspots",
     xlab = "Wavelength (nm)", ylab = "Reflectance", ylim = c(0, max_reflectance))
polygon(c(tanData_filtered$wl, rev(tanData_filtered$wl)), 
        c(b_spline_avg + b_spline_sd, rev(b_spline_avg - b_spline_sd)), 
        col = adjustcolor(b_spline_rgb, alpha.f = 0.2), border = NA)
lines(tanData_filtered$wl, b_spline_avg, col = b_spline_rgb, lwd = 4)
...
# Average B Spline with Hotspots in PDF
plot(tanData_filtered$wl, b_spline_avg, type = "l", col = b_spline_rgb, lwd = 4,
     main = "Average B Spline with Variation Hotspots",
     xlab = "Wavelength (nm)", ylab = "Reflectance", ylim = c(0, max_reflectance))
polygon(c(tanData_filtered$wl, rev(tanData_filtered$wl)), 
...
### Step 4b: Plot Natural Spline Mean and B Spline Mean with Species Overlaid
...
# Plot B Spline Mean with Species Overlaid
plot(tanData_filtered$wl, b_spline_avg, type = "l", col = b_spline_rgb, lwd = 4,
     main = "B Spline Mean with Species Overlaid",
     xlab = "Wavelength (nm)", ylab = "Reflectance", ylim = c(0, max_reflectance))
for (i in seq_along(spline_results)) {
  species_rspec <- as.rspec(data.frame(wl = tanData_filtered$wl, reflectance = predict(spline_results[[i]]$b_spline_fit)), lim = c(300, 700))
  species_rspec <- procspec(species_rspec, fixneg = "zero")  # Correct any negative values
  species_rgb <- spec2rgb(species_rspec)
  lines(tanData_filtered$wl, predict(spline_results[[i]]$b_spline_fit), col = adjustcolor(species_rgb, alpha.f = 0.3))
}
lines(tanData_filtered$wl, b_spline_avg, col = b_spline_rgb, lwd = 4)
...
# Plot B Spline Mean with Species Overlaid in PDF
plot(tanData_filtered$wl, b_spline_avg, type = "l", col = b_spline_rgb, lwd = 4,
     main = "B Spline Mean with Species Overlaid",
     xlab = "Wavelength (nm)", ylab = "Reflectance", ylim = c(0, max_reflectance))
for (i in seq_along(spline_results)) {
...
### Step 4b Make a pair of plots of the natural and B Spline where the spline is colored as a spec that is converted to its hsv value (make sure no negative values)
...
# Convert the B Spline to an rspec object and then to an RGB hex code
b_spline_rspec <- as.rspec(data.frame(wl = tanData_filtered$wl, reflectance = b_spline_avg), lim = c(300, 700))
b_spline_rgb <- spec2rgb(b_spline_rspec)

# Plot the natural and B Splines side by side in R Markdown
par(mfrow = c(1, 2))  # Arrange plots side by side
...
# Plot B Spline with Species Overlaid
plot(tanData_filtered$wl, b_spline_avg, type = "l", col = b_spline_rgb, lwd = 4,  # Increased thickness
     main = "B Spline with Species Overlaid",
     xlab = "Wavelength (nm)", ylab = "Reflectance", ylim = c(0, max_reflectance))

# Loop through each species to plot their splines with slightly more visible transparency
for (species in names(spline_results)) {
  species_spline_rspec <- as.rspec(data.frame(wl = tanData_filtered$wl, reflectance = predict(spline_results[[species]]$b_spline_fit)), lim = c(300, 700))
  species_rgb <- spec2rgb(species_spline_rspec)
  
  # Adjust transparency to make species lines slightly more visible
  species_rgb_alpha <- adjustcolor(species_rgb, alpha.f = 0.4)  # Adjusted alpha for more visibility
  
  lines(tanData_filtered$wl, predict(spline_results[[species]]$b_spline_fit), col = species_rgb_alpha)
}

# Re-plot the B Spline on top to ensure it stands out
lines(tanData_filtered$wl, b_spline_avg, col = b_spline_rgb, lwd = 4)  # Increased thickness

# Save the plots to a PDF as well
pdf("natural_and_b_splines_with_species_overlay.pdf", width = 12, height = 8)
par(mfrow = c(1, 2))  # Arrange plots side by side in the PDF
...
# Plot B Spline with Species Overlaid in PDF
plot(tanData_filtered$wl, b_spline_avg, type = "l", col = b_spline_rgb, lwd = 4,
     main = "B Spline with Species Overlaid",
     xlab = "Wavelength (nm)", ylab = "Reflectance", ylim = c(0, max_reflectance))
for (species in names(spline_results)) {
  species_rgb_alpha <- adjustcolor(spec2rgb(as.rspec(data.frame(wl = tanData_filtered$wl, reflectance = predict(spline_results[[species]]$b_spline_fit)), lim = c(300, 700))), alpha.f = 0.4)
  lines(tanData_filtered$wl, predict(spline_results[[species]]$b_spline_fit), col = species_rgb_alpha)
}
lines(tanData_filtered$wl, b_spline_avg, col = b_spline_rgb, lwd = 4)
...
for (species in names(spline_results)) {
  ...
  # Plot original data and B Spline fit
  plot(tanData_filtered$wl, tanData_filtered[[species]], main = paste("B Spline Fit for", species),
       xlab = "Wavelength (nm)", ylab = "Reflectance", type = "o", col = color)
  lines(tanData_filtered$wl, predict(result$b_spline_fit), col = "green", lwd = 2)
  mtext(paste("Adj R2:", round(result$quartic_adj_r2, 3), "AIC:", round(result$quartic_aic, 2)),
        side = 3, line = 0.5, cex = 0.7, col = "blue")
}
...
spline_table <- data.frame(
  Species = names(spline_results),
  Natural_Spline_Coefficients = sapply(spline_results, function(x) paste(x$natural_coef, collapse = ", ")),
  b_spline_Coefficients = sapply(spline_results, function(x) paste(x$quartic_coef, collapse = ", ")),
  Natural_Adj_R2 = sapply(spline_results, function(x) round(x$natural_adj_r2, 3)),
  Quartic_Adj_R2 = sapply(spline_results, function(x) round(x$quartic_adj_r2, 3)),
  Natural_AIC = sapply(spline_results, function(x) round(x$natural_aic, 2)),
  Quartic_AIC = sapply(spline_results, function(x) round(x$quartic_aic, 2))
