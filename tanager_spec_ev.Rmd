...
# Reconstructing splines from scratch
...
# gls spline fit example

TanCyo <- spline_results[["TanCyo"]]
gls_spline_reconstruction <- predict(TanCyo$reflectance_spline_fit)

# Spline reconstruction using only spline coefficients and our basis

bspline_matrix <- bSpline(tanData_filtered$wl, df = 38)
...
TanCyo_spline_coef <- TanCyo$reflectance_coef

# Must make coefficients from 2:39 as bSpline function does not create an intercept coefficient
coef_spline_reconstruction <- predict(bspline_matrix, coef = as.numeric(c(TanCyo_spline_coef[2:39])))

coef_spline_reconstruction <- cbind(coef_spline_reconstruction, seq(351,700,1))
coef_spline_reconstruction <- data.frame(coef_spline_reconstruction)

plot(y = coef_spline_reconstruction$coef_spline_reconstruction, x = coef_spline_reconstruction$V2, main = "Reconstructed Spline", xlab = "Wavelength (nm)", ylab = "Index?")
