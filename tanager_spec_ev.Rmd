...
```{r, message=FALSE, warning=FALSE}
  ...
  fit_splines <- function(wl, reflectance) {
    # Fit natural b spline
    natural_spline_fit <- lm(reflectance ~ ns(wl, df = 8))
    
    # Fit b spline
    natural_spline_fit_restrained_knots <- lm(reflectance ~ ns(wl, Boundary.knots = c(300,700),
                                                               knots = seq(350,650, by = 50)))
    
    # Extract coefficients
    natural_coef <- round(coef(natural_spline_fit), 2)
    natural_restknots_coef <- round(coef(natural_spline_fit_restrained_knots), 2)
    
    # Calculate goodness-of-fit metrics
    natural_adj_r2 <- summary(natural_spline_fit)$adj.r.squared
    natural_restknots_adj_r2 <- summary(natural_spline_fit_restrained_knots)$adj.r.squared
    
    natural_aic <- AIC(natural_spline_fit)
    natural_restknots_aic <- AIC(natural_spline_fit_restrained_knots)
    
    return(list(
      natural_spline_fit = natural_spline_fit,
      natural_restknots_spline_fit = natural_spline_fit_restrained_knots,
      natural_coef = natural_coef,
      natural_restknots_coef = natural_restknots_coef,
      natural_adj_r2 = natural_adj_r2,
      natural_restknots_adj_r2 = natural_restknots_adj_r2,
      natural_aic = natural_aic,
      natural_restknots_aic = natural_restknots_aic
    ))
  }
  ...
  spline_table <- data.frame(
    Species = names(spline_results),
    Natural_Spline_Coefficients = sapply(spline_results, function(x) paste(x$natural_coef, collapse = ", ")),
    natural_restknots_Spline_Coefficients = sapply(spline_results, function(x) paste(x$natural_restknots_coef, collapse = ", ")),
    Natural_Adj_R2 = sapply(spline_results, function(x) round(x$natural_adj_r2, 3)),
    natural_restknots_Adj_R2 = sapply(spline_results, function(x) round(x$natural_restknots_adj_r2, 3)),
    Natural_AIC = sapply(spline_results, function(x) round(x$natural_aic, 2)),
    natural_restknots_AIC = sapply(spline_results, function(x) round(x$natural_restknots_aic, 2))
  )
...
spline_table_df_ordered <- cbind(degrees_of_freedom, spline_table_df_ordered)
  ...
  fit_splines <- function(wl, reflectance) {
    # Fit natural b spline
    natural_spline_fit <- lm(reflectance ~ ns(wl, df = 9))
