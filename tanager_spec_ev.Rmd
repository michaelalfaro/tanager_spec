...
BEAST.variance.rates %>% ggplot(aes(x = coef.names, y = BEAST, fill = BEAST)) + geom_bar(stat = "identity") + theme(axis.text.x = element_text(angle = 0, hjust = 0.5)) + labs(x = "Coefficients", y = expression(paste("Variance", sep = " ", (sigma^{2}))), title = "Evolutionary Rates of Spline Coefficients", subtitle = "Method = BEAST") + theme_bw() + scale_fill_viridis_c()
```
...
for (i in 1:length(coef.names)){
  ...
  x <- rep(coefficients_names.1[i], 39)
