...
rownames(wl_summary_stats) <- wl_bins
...
# Vector to match the wavelength bins to their respective color in the visible spectrum
colors_for_plots <- c(rep("violet", 8), rep("purple", 2), rep("blue",5), rep("green", 7), rep("yellow", 2), rep("orange", 3), rep("orangered",5), rep("red", 8))

# Boxplot of the reflectance data for each wavelength bin
boxplot(data = wl_dataframe, Reflectance_Values~Wavelength_Bins, col = colors_for_plots, main = "Summary Statistics for Thraupidae Crown Reflectance (51 Species)", xlab = "Wavelength Bins (10nm Intervals)", ylab = "Reflectance (%)")
...
# Side-by-side plot comparison of our gls reconstructed plot and the plot from Step 5
...
# Verifying that these two functions produce identical matrices (output should be TRUE)
...
str(coef_spline_reconstruction)
...
test <- cbind(300:700,predict(bspline_matrix_bSpline, coef = as.numeric(c(TanCyo_spline_coefficients[2:39]))))


plot(300:700, test[,2])

x <- TanCyo_spline_coefficients[2:39]
print(TanCyo_spline_coefficients[21])
x[20] <- x[20]+1

test2 <- cbind(300:700, predict(bspline_matrix_bSpline, coef = x))
plot(300:700, test2[,2])


par(mfrow=c(1,2))
plot(300:700, test[,2], pch = 19)
plot(300:700, test2[,2], pch = 19)
par(mfrow=c(1,1))

y <- cbind(predict(bspline_matrix_bSpline, coef = as.numeric(c(TanCyo_spline_coefficients[2:39]))),predict(bspline_matrix_bSpline, coef = x))
y <- cbind(300:700,y)
